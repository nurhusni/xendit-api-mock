...
- `ALLOW_FAILED_DISBURSEMENT_CALL` (optional, default `false`; set `true` to make `POST /xendit/disbursements` return HTTP 500)
- `DISABLE_CALLBACKS` (optional, default `false`; set `true` to skip callback POSTs after `POST /xendit/disbursements`)
